...
const Profile = () => {
  ...
  return (
    <div className={styles.user_profile} onClick={toggleOptionsVisible}>
      ...
      <p className={styles.user_profile__name}>{userName}</p>
      <ul
        className={cx({ options: true, 'options--visible': showOptions })}
        onMouseLeave={toggleOptionsVisible}
      >
        <li className={styles.options__item} onClick={() => dispatch(logOut())}>
          Выйти
        </li>
      </ul>
    </div>
