...
import { editContact, createContact, deleteContact } from '../../state/contactsSlice'
...
const ContactEditor: React.FC<Props> = ({ contact, onCancel }) => {
    ...
    const onSubmit = async (data: Contact) => {
        if (contact) {
            const result = await dispatch(editContact({ contact: data, accessToken }));
            ...
            const result = await dispatch(createContact({ contact: data, accessToken }));
    ...
    return (
        <div className={styles.editor_container}>
            ...
            <ContactForm contact={contact} onCancel={onCancel} onSubmit={onSubmit} onDelete={onDelete}/>
        </div>
